...
import { Mic, Send, Bot, User, CheckCircle2, Loader2, LogOut } from "lucide-react";
...
export default function ChatInterface({ userName, userImage }: { userName?: string | null, userImage?: string | null }) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "archivist",
      content: `Welcome back, ${userName || "Commander"}! I am Antigravity, your Life OS archivist. Tap the microphone and tell me what's on your mind.`,
    },
  ]);
    ...
    <main className="flex flex-col h-screen bg-slate-950 text-slate-50 font-sans selection:bg-indigo-500/30">
      ...
      <header className="flex items-center justify-between p-6 border-b border-slate-800/60 bg-slate-900/50 backdrop-blur-md">
        ...
        <div className="flex items-center gap-4">
           {userImage && <img src={userImage} alt="User Avatar" className="w-8 h-8 rounded-full ring-2 ring-indigo-500/50" />}
           <button onClick={() => signOut()} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm bg-slate-800 hover:bg-slate-700 transition">
